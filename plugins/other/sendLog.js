...
export class sendLog extends plugin {
  ...
  async sendLog() {
    try {
      const match = this.e.msg.match(
        /^#(运行|错误|追踪|调试|trace|debug)?日志(\d+)?(.*)$/i
      );
      ...
      const { logFile, filterLevel, logName } =
        await this.getLogConfig(logType);
  ...
  buildBatchForwardData(
    ...
    if (batchNum === 1) {
      const headerInfo = this.buildHeaderInfo(
        logName,
        keyWord,
        filterLevel,
        timestamp,
  ...
  async findLogFile(prefix = 'app') {
    try {
      ...
      const todayLogFile = path.join(
        this.logDir,
        `${prefix}.${currentDate}.log`
      );
      ...
      const logFiles = files
        .filter(
          (file) => file.startsWith(`${prefix}.`) && file.endsWith('.log')
        )
